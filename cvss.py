...
def main():
    ...
    if uploaded_file:
        ...
        st.pyplot(plt.gcf())
                ...
                # 🧠 CWE-Based Vulnerability Trends
        st.subheader("📉 Vulnerability Trends by Top CWEs Over Time")

        # Get top 5 most frequent CWEs
        top_cwes = df["cwe"].value_counts().head(5).index.tolist()

        cwe_trend_df = df[df["cwe"].isin(top_cwes)].groupby(["Month", "cwe"])["id"].count().reset_index()
        cwe_trend_df["Month"] = cwe_trend_df["Month"].astype(str)

        plt.figure(figsize=(12, 5))
        sns.lineplot(data=cwe_trend_df, x="Month", y="id", hue="cwe", marker="o")
        plt.xticks(rotation=45)
        plt.xlabel("Month")
        plt.ylabel("Vulnerability Count")
        plt.title("Top CWE-Based Vulnerability Trends Over Time")
